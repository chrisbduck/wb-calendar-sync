...
- Google can return special all-day events, especially birthdays without a birth year, with dates like `0000-06-01`. Python cannot parse year zero, so sync should skip those events and log their ID/title instead of failing the whole run with `year 0 is out of range`.
